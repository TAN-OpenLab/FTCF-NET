...
class FTCF_TP_Block(tf.keras.layers.Layer):
    def __init__(self, filters=32, kernel_size=(3, 3, 3), strides=(1, 1, 1), weight_decay=0.005):
        super(FTCF_TP_Block, self).__init__()
        self.filters = filters
        self.kernel_size = kernel_size
        self.strides = strides
        self.weight_decay = weight_decay
    ...
    def build(self, input_shape):
        ...
        self.conv2d_list = []
        self.shape = input_shape
        self.small_full_temproal_network = [FuseFullTemproalSubNetwork(filters=self.T) for _ in range(self.channels)]
...
class FTCF_SP_Block(tf.keras.layers.Layer):
    def __init__(self, filters=32, kernel_size=(3, 3, 3), strides=(1, 1, 1), weight_decay=0.005):
        super(FTCF_SP_Block, self).__init__()
        self.filters = filters
        self.kernel_size = kernel_size
        self.strides = strides
        self.weight_decay = weight_decay
...
class FTCF_TP_Simple(tf.keras.layers.Layer):
    def __init__(self, filters=32, kernel_size=(3, 3, 3), strides=(1, 1, 1), weight_decay=0.005):
        super(FTCF_TP_Simple, self).__init__()
        self.filters = filters
        self.kernel_size = kernel_size
        self.strides = strides
        self.weight_decay = weight_decay
    ...
    def call(self, input_tensor, training=True):
        x = tf.transpose(input_tensor, (4, 0, 1, 2, 3))
        for i in range(self.channels):
            t = self.conv2d_list[i][0](x[i])
            t = self.conv2d_list[i][1](t, training=training)
            t = self.mish(t)
            x = tf.tensor_scatter_nd_update(x, [[i]], tf.expand_dims(t, axis=0))
        x = tf.transpose(x, (1, 2, 3, 4, 0))
